...
class Segment:
    def __init__(self, start, end):
        ...
        self.end = end
    def length(self):
        return round(((self.end[1] - self.start[1]) **2  + (self.end[0] - self.start[0]) **2)**0.5, 2)

    def x_axis_intersection(self):
        if self.start[0] < 0 and self.end[0] > 0:
            return True
        elif self.start[0] == 0 or self.end[0] == 0:
            return True
        else:
            return False
    def y_axis_intersection(self):
        if self.start[1] < 0 and self.end[1] > 0:
            return True
        elif self.start[1] == 0 or self.end[1] == 0:
            return True
        else:
            return False
