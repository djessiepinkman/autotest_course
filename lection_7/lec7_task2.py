...
class PersonInfo:
    ...
    def short_name(self):
        ...
        new_name = list[1] + ' ' + list[0][0] + '.'
        return new_name

    def path_deps(self):
        new_subdivision = []
        for i in self.subdivision:
            new_subdivision.append(i)
            new_subdivision.append(' --> ')
        new_subdivision.pop()
        new_subdivision = ''.join(new_subdivision)
        return new_subdivision

    def new_salary(self):
        self.subdivision = ''.join(self.subdivision)

        def summa_top3(subdivision):
            t = ([(subdivision.count(i), i) for i in subdivision])
            list = (sorted(set(t))[::-1][:3])
            summa = list[0][0] + list[1][0] + list[2][0]
            return summa

        salary = 1337 * self.age * summa_top3(self.subdivision)
        return salary
