...
def everything_for_your_cat(cats_data):
    people = {}  # Пустой словарь. В итоге в словарь будет добавлено: ключ - фамилия и имя владельца котика с двоеточием, значение - имя котика и возраст через запятую
    lst = []  # пустой список в который в цикле добавлются построчно строки из cats_data
    our_str = ''  # строка, которую в итоге нужно вывести (пока пустая)
    for i in range(len(cats_data)):
        lst = list(cats_data[i])
        FI_chel = str(lst[2]) + ' ' + str(
            lst[3]) + ':'  # в переменную записывается фамилия и имя владельца котика с двоеточием
        inf_cat = str(lst[0]) + ', ' + str(lst[1])  # в переменную записывается имя котика и возраст через запятую
        if (FI_chel) not in people:  # Если фамилия и имя владельца котика с двоеточием не в словаре
            people[
                FI_chel] = inf_cat  # То записываем её в словарь как ключ, а значение - имя котика и возраст через запятую
        else:  # Если фамилия и имя владельца котика с двоеточием уже есть  в словаре
            people[
                FI_chel] += '; ' + inf_cat  # То добавляем к имеющемуся в словаре значении ; и новое  имя котика и возраст через запятую

    for k, v in people.items():  # Проходим по словарю people
        our_str += str(k) + ' ' + str(
            v) + '\n'  # В переменную записываем построчно пару ключ значение с переносом строки
